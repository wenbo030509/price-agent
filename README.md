...
|---|---|---|---|
| **适用场景** | 单商品比价、简单查询 | 多商品对比、复杂分析、**推荐+对比混合意图** | "推荐游戏手机"、"5000以内性价比手机"、"骁龙处理器手机有哪些" |
| **触发条件** | 默认兜底 | 多商品 / 对比词 / 推荐触发+对比证据 | 场景词/预算词/处理器词 + 无明确型号 |
...
- **意图分类路由 🆕**：自动识别 query 意图（推荐/查价/对比/混合），路由到最优执行模式。支持混合意图——"推荐游戏手机，再和 iPhone 15 对比"自动拆解为先推荐筛选、再精确比价的两步 Plan-Execute
